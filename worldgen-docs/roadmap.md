...
- [ ] **`--bake-region` mode** (`src/bake.rs`, `src/args.rs`, dispatched from `src/main.rs`)
  - Bake one region (or a range) into an existing world's `region/` dir.
...
- [ ] **Fixed/explicit projection origin** — stop deriving `web_mercator` origin from
  per-run bbox center; thread a fixed `(origin_lat, origin_lon)` through
  (`main.rs`, `CoordTransformer::with_projection`) so independent bakes stay aligned.
- [ ] **Machine-readable output** — emit JSON to stdout (regions written, timings,
  errors) for the plugin to parse.
...
- [ ] **`VoidChunkGenerator`** — empty-air generator for the arnis world; only fills
  gaps not yet baked (chunk storage is consulted first, so a baked region loads verbatim).
- [ ] **`RegionBaker`** — bounded worker pool that runs `arnis --bake-region …` as a
  subprocess, parses the JSON result, and guards against baking a currently-loaded region.
...
- [ ] **`ArnisCommand`** — `/arnis status | prewarm <radius> | reload | goto <lat> <lng>`.
...
- [ ] **Plugin scaffolding** — `plugin.yml`, `config.yml` (origin lat/lon, scale,
  prefetch radius, arnis binary path, worker count, world name), world bootstrap via
  `WorldCreator(...).generator(voidGen)`, loading `arnis.json`.
...
- [ ] **Logging** — log to the server console on plugin enable/load; log the
...
- [ ] **Phase 0 — Rust bake mode.** `--bake-region` + fixed origin; bake `r.0.0.mca`
  into an empty Java world folder; verify against an equivalent full-run over the same bbox.
- [ ] **Phase 1 — Minimal plugin.** Paper void generator + `/arnis prewarm` invoking
  the subprocess; manual, synchronous.
