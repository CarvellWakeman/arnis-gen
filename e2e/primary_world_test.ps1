...
.DESCRIPTION
  ...
  additional worlds on STARTUP" errors do NOT occur, the arnis world comes up, and
  the spawn region bakes. Complements smoke_test.ps1, which covers the default
  (separate-world) setup.
...
$FAILS = @("Could not set generator", "Cannot create additional worlds", "Failed to create/load arnis world")
$outcome = "timeout"; $genError = $false
$t = [System.Diagnostics.Stopwatch]::StartNew()
while ($t.Elapsed.TotalSeconds -lt $TimeoutSec) {
    $line = $null
    if ($queue.TryDequeue([ref]$line)) {
        $sw.WriteLine($line); $sw.Flush()
        foreach ($f in $FAILS) { if ($line -match [regex]::Escape($f)) { $genError = $true; $outcome = "gen-error: $f" } }
...
$pass = ($outcome -eq "ok") -and (-not $genError) -and $regionOk
if (-not $KeepRun -and $pass) { Remove-Item $run -Recurse -Force -ErrorAction SilentlyContinue }
Write-Host "[test] outcome=$outcome  generator-error=$genError  region-file=$regionOk"
if ($pass) {
    Write-Host "[test] PASS: arnis loaded as primary world, no generator error, spawn baked." -ForegroundColor Green
    exit 0
} else {
